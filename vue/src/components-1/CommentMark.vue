<template>
  <div class="comment-mark-container">
...
import CommentSub from "./CommentMarkSub.vue";
...
const getSubComment = () => {
  url = `/comment/get/sub?idxComment=${id}`;
  proxy.$axios
    .get(url)
    .then((response) => {
      if (response.status === 200 && response.data.code === 1) {
        subcomment.value = response.data.data;
      }
    })
    .catch((error) => {
